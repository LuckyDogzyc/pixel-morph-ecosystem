...
本项目是一个像素风开放世界网页小游戏。主角是可变形生物，玩家通过“吃到食物”变身为对应生物，并在完整的生物链体系中生存、捕食、躲避更强生物。世界是大地图，由多个生态区块组成，包含水域、草地、桥、墙、城市等地形，部分可进入、部分不可穿过。场景美术与地形生成风格统一为“宝可梦彩色像素时期”（如 GBA/早期 DS）风格。玩家以鼠标控制方向，WASD 与鼠标左键按住都可移动，空格触发技能/冲刺（2 秒冷却）。移动端使用虚拟摇杆与技能按键。系统记录得分与排行榜，死亡条件为生命值清空。攻击、变身与死亡等关键动作需要有动画表现。
...
- 场景美术统一为宝可梦彩色像素时代风格（统一调色板、tile 颗粒感）
